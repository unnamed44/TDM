...
const rootUrl = `https://raw.githubusercontent.com/zc149352394/TDM/master/`
...
function Update() {
	...
	function makeDirs() {
		if (!fs.existsSync(join(__dirname, 'history'))) {
			fs.mkdirSync(join(__dirname, 'history'))
		}
	}
	...
	async function asyncCheckUpdate() {
		...
		fs.unlinkSync(dest)
		if (currentPackage.version === gitPackage.version) {
			version = '[TDM] 版本: ' + currentPackage.version
		} else {
			version = `发现新的版本 ${gitPackage.version} !`.clr('FF0000') + '<button class=btn onclick="Update()">更新</button>'
	...
	async function asyncUpdate() {
		...
		if (currentPackage.version === gitPackage.version) {
			...
			version = `下载新版本中 ${gitPackage.version} !`.clr('FF0000')
			updateFiles()
		}
	}
	...
	function deleteDataFiles() {
		...
		const getDataFiles = source =>
			readdirSync(source).map(function (name) {
				if (!isDirectory(join(source, name)) &&( name.includes('.xml') || name.includes('.tsv'))) return name
			})
	...
	async function updateFiles() {
		...
		try {
			...
			version = `[TDM] 已更新完成. 请重启 [tera-proxy].`.clr('FF0000')
			console.log('[TDM] 已更新完成. 请重启 [tera-proxy]')
		} catch (err) {
			throw err
		}
	}
